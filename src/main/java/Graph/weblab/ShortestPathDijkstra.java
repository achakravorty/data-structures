...
public class ShortestPathDijkstra {
    ...
    public static List<Vertex> shortestPath(Graph g, Vertex a, Vertex b) {
        ...
        //first do dijkstra to get shortest distances
        int[] res = new int[g.getAllVertices().size()]; //the weight of best path so far
        boolean[] visited = new boolean[g.getAllVertices().size()]; //keep track of visited vertices
        int[] parent = new int[g.getAllVertices().size()]; //keep track of parent vertex

        AdaptablePQ pq = new AdaptablePQ();

        //initialise nodes, D[s] = 0 and D[v] = infinity
        for (Vertex v : g.getAllVertices()) {
            if (v.getId() != a.getId()) {
                res[v.getId()] = Integer.MAX_VALUE;
            }
            pq.insertOrReplace(v, res[v.getId()]); //add entries to pq
        }
        ...
        while (!pq.isEmpty()) {
            ...
            for (VertexNumPair vnp : u.getNeighbours()) {
                Vertex v = vnp.getVertex();
                int edgeWeight = vnp.getNum();

                //edge relaxation
                //if D[u] + w(u,v) < D[v] then D[v] = D[u] + w(u,v)
                if (!visited[v.getId()] && res[u.getId()] + edgeWeight < res[v.getId()]) {
                    res[v.getId()] = res[u.getId()] + edgeWeight;
                    parent[v.getId()] = u.getId();  //add to parent list
                    pq.insertOrReplace(v, res[v.getId()]);
                }
            }
        }

        //second make a way to keep track of parent node for d[v]
//        System.out.println(Arrays.toString(res)); // this is correct list and dijkstra implementation is right
//        System.out.println(Arrays.toString(parent));


        //backtrack from vertex b to a to get shortest path
        //doing this only if path exists
        if (res[b.getId()] != Integer.MAX_VALUE){
            Vertex pos = b;
            path.add(0, pos);
            while (!pos.equals(a)) {
                int parentOfVertex = parent[pos.getId()];
                Vertex p = g.getVertex(parentOfVertex);
                path.add(0, p);
                pos = p;
            }
        }
